...
$reservasi_per_bulan = fetch_all("
    SELECT DATE_FORMAT(created_at,'%b %Y') bulan, COUNT(*) total
    FROM reservasi
...
Highcharts.chart('reservasiChart',{
    chart:{type:'line'},
    title:{text:null},
    xAxis:{categories:<?= json_encode(array_column($reservasi_per_bulan,'bulan')) ?>},
    series:[{
        data:<?= json_encode(array_map('intval',array_column($reservasi_per_bulan,'total'))) ?>,
        color:'#d4af37'
    }],
